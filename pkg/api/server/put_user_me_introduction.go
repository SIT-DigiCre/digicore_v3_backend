...
func (s *server) PutUserMeIntroduction(ctx echo.Context) error {
	var requestBody api.ReqPutUserMeIntroduction
	...
	res, err := users.PutUserMeIntroduction(ctx, &dbTranisactionClient, requestBody)
	if err != nil {
		return response.ErrorResponse(ctx, err)
	}
