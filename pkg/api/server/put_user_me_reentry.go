...
func (s *server) PutUserMeReentry(ctx echo.Context) error {
	var requestBody api.ReqPutUserMeReentry
	...
	res, err := user.PutUserMeReentry(ctx, &dbTranisactionClient, requestBody)
	if err != nil {
		return response.ErrorResponse(ctx, err)
	}
