...
func (s *server) DeleteAdminDeleteExpiredUserPrivateProfiles(ctx echo.Context) error {
	...
	res, err := admin.DeleteExpiredUserPrivateProfiles(ctx, &dbTransactionClient)
	if err != nil {
		return response.ErrorResponse(ctx, err)
	}
