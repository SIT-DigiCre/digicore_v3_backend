...
func (s *server) DeleteActivityRecordRecordId(ctx echo.Context, recordId string) error {
	...
	res, err := activity.DeleteActivityRecordRecordId(ctx, &dbTransactionClient, recordId)
	if err != nil {
		return response.ErrorResponse(ctx, err)
	}
