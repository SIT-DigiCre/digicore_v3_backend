...
func (s *server) PostActivityCheckoutUserId(ctx echo.Context, userId string) error {
	var requestBody api.ReqPostActivityCheckout
	...
	res, err := activity.PostActivityCheckoutUserId(ctx, &dbTransactionClient, userId, requestBody)
	if err != nil {
		return response.ErrorResponse(ctx, err)
	}
