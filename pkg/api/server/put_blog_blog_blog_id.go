...
func (s *server) PutBlogBlogBlogId(ctx echo.Context, blogId string) error {
	var requestBody api.ReqPutBlogBlogBlogId
	...
	res, err := blog.PutBlogBlogBlogId(ctx, &dbTranisactionClient, blogId, requestBody)
	if err != nil {
		return response.ErrorResponse(ctx, err)
	}
