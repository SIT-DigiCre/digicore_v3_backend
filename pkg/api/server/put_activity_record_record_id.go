...
func (s *server) PutActivityRecordRecordId(ctx echo.Context, recordId string) error {
	var requestBody activity.ActivityRecordUpdateRequest
	...
	res, err := activity.PutActivityRecordRecordId(ctx, &dbTransactionClient, recordId, requestBody)
	if err != nil {
		return response.ErrorResponse(ctx, err)
	}
