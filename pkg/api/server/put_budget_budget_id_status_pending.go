...
func (s *server) PutBudgetBudgetIdStatusPending(ctx echo.Context, budgetId string) error {
	var requestBody api.ReqPutBudgetBudgetIdStatusPending
	...
	res, err := budget.ReqPutBudgetBudgetIdStatusPending(ctx, &dbTranisactionClient, budgetId, requestBody)
	if err != nil {
		return response.ErrorResponse(ctx, err)
	}
