...
func (s *server) PutAdminChangeStudentNumber(ctx echo.Context) error {
	var requestBody api.ReqPutAdminChangeStudentNumber
	...
	res, err := admin.PutAdminChangeStudentNumber(ctx, &dbTransactionClient, requestBody)
	if err != nil {
		return response.ErrorResponse(ctx, err)
	}
