...
func (s *server) PostActivityCheckin(ctx echo.Context) error {
	var requestBody api.ReqPostActivityCheckin
	...
	res, err := activity.PostActivityCheckin(ctx, &dbTransactionClient, requestBody)
	if err != nil {
		return response.ErrorResponse(ctx, err)
	}
