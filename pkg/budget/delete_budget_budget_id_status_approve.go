...
func DeleteBudgetBudgetIdStatusApprove(ctx echo.Context, dbClient db.TransactionClient, budgetId string) (api.BlankSuccess, *response.Error) {
	...
	err = deleteApproveBudget(dbClient, budgetId)
	if err != nil {
		return api.BlankSuccess{}, err
	}
...
func deleteApproveBudget(dbClient db.TransactionClient, budgetId string) *response.Error {
	params := struct {
