...
`GET /user/me` のレスポンスには `claims` 配列が含まれ、ユーザーが持つ役職 claim（`account` / `infra`）が列挙されます。
...
| user_id | 名前 | 学籍番号 | is_member | 役職 claim |
|---------|------|----------|-----------|-----------|
| `11111111-1111-1111-1111-111111111111` | 田中太郎 | 20230001 | `true` | `account` |
| `22222222-2222-2222-2222-222222222222` | 佐藤花子 | 20230002 | `true` | なし |
| `33333333-3333-3333-3333-333333333333` | 山田次郎 | 20230003 | `true` | `infra` |
| `44444444-4444-4444-4444-444444444444` | 鈴木美咲 | 20230004 | `false` | なし |
| `55555555-5555-5555-5555-555555555555` | 高橋健太 | 20230005 | `true` | なし |
| `66666666-6666-6666-6666-666666666666` | 伊藤一真 | 20230006 | `false` | なし |
| `77777777-7777-7777-7777-777777777777` | 中村直樹 | 20230007 | `false` | `infra` |
...
```bash
./scripts/gen_jwt.sh 66666666-6666-6666-6666-666666666666
```

**非部員かつ infra claim あり（中村直樹）:**

```bash
./scripts/gen_jwt.sh 77777777-7777-7777-7777-777777777777
```
...
#### `GET /user/me` の claims 確認

```bash
# account ユーザー → ["account"]
curl -s -H "Authorization: Bearer <田中太郎のトークン>" \
  http://localhost:8080/user/me | jq .claims
...
# infra ユーザー → ["infra"]
curl -s -H "Authorization: Bearer <山田次郎のトークン>" \
  http://localhost:8080/user/me | jq .claims
```
