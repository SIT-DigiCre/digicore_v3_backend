...
部室への入退室管理機能を実装します。場所は`place`カラムで管理し、部室に限定しません。チェックインとチェックアウトは同じテーブル`activities`の1レコード内で`checked_in_at`および`checked_out_at`として管理し、最新レコードの`checked_out_at`がNULLかどうかで在室状況を判定します。
...
- 在室判定は最新のチェックインレコードの`checked_out_at`がNULLかどうかで判定できるためシンプル
